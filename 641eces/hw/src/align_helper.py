...
def local_align(x, y, score=ScoreParam(10, -5, -7), algo="N-W"):
...
def rnafold(rna):
    ...
    for k in range(5, n):  # interval length
        for i in range(n - k):  # interval start
            j = i + k  # interval end
...
def rna_backtrace(Arrows):
    ...
    print(f"Matched Pairs = {', '.join([ij for ij in Pairs])}")
    return Pairs