...
handle = Entrez.esearch(
    db="nucleotide",
    term="16S rRNA[gene] AND streptococcus[ORGN] AND Manfredo AND genome",
...
for feature in record.features:
    if (
        feature.type == "gene" or feature.type == "rRNA"
    ):  # If the feature is a Gene or rRNA then
        if (
            "gene" in feature.qualifiers
        ):  # This looks to see if /gene= exists in the second column
            if (
                feature.qualifiers["gene"][0] == "16S rRNA"
            ):  # If the first occurrence of gene is /gene="16S rRNA"

                if (
                    str(feature.location) not in locations
                ):  # If the feature location is not already in the locations list
                    print(feature.location)
                    locations.append(
                        str(feature.location)
                    )  # append the location to the locations list
                    print(locations)
                    sixteen_s.append(
                        feature
                    )  # append the feature itself to a list of 16S features
                    seqs.append(
                        feature.extract(record.seq)
                    )  # We can also extract just the sequences
...
for i in range(len(seqs)):
    ...
    )  # This outputs the record ID, description, location of the sequence and sequence itself to a file
output_handle.close()