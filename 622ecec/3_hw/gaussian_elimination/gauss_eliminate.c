...
void gauss_eliminate_using_omp(Matrix U)
{
    #pragma omp parallel num_threads(4)
    {
        ...
        int i, j, k;
        unsigned int n_rows = U.num_rows;
        unsigned int n_cols = U.num_columns;
        float* mat = U.elements;
        ...
        #pragma omp for collapse(2)
            for (i = (k + 1); i < n_rows; i++) {
                for (j = (k + 1); j < n_cols; j++)
                    mat[n_cols * i + j] = mat[n_cols * i + j] - (mat[n_cols * i + k] * mat[n_cols * k + j]);	/* Elimination step */
            }
