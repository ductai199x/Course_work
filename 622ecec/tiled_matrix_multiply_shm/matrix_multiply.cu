...
{
    /* Create and populate the n x n matrices */
    srand(time(NULL));
    matrix_t M = allocate_matrix(MATRIX_SIZE, MATRIX_SIZE, 1);	
	matrix_t N = allocate_matrix(MATRIX_SIZE, MATRIX_SIZE, 1);

    struct timeval start, stop;	
    fprintf(stderr, "\nMultiplying %d x %d matrices on CPU using naive version\n", MATRIX_SIZE, MATRIX_SIZE);
	gettimeofday(&start, NULL);

    matrix_t P_reference_naive = allocate_matrix(MATRIX_SIZE, MATRIX_SIZE, 0);  
//    compute_gold_naive(M, N, P_reference_naive);
    ...
    matrix_t P_reference_blocked = allocate_matrix(MATRIX_SIZE, MATRIX_SIZE, 0);  
//    compute_gold_blocked(M, N, P_reference_blocked);
    ...
    matrix_t P_device = allocate_matrix(MATRIX_SIZE, MATRIX_SIZE, 0);
...
void matrix_multiply_on_device(matrix_t M, matrix_t N, matrix_t P)
{
    ...
    fprintf(stderr, "Setting up a %d x %d grid of thread blocks\n", 
            (Pd.width + TILE_SIZE - 1)/TILE_SIZE, (Pd.height + TILE_SIZE - 1)/TILE_SIZE);
	dim3 grid((Pd.width + TILE_SIZE - 1)/TILE_SIZE, (Pd.height + TILE_SIZE - 1)/TILE_SIZE);

	/* Launch kernel */
	matrix_multiply_kernel_naive<<< grid, threads >>>(Pd.elements, Md.elements, Nd.elements, MATRIX_SIZE);
    ...
    fprintf(stderr, "\nKernel now uses shared memory\n");
    gettimeofday(&start, NULL);
    
    /* Launch kernel using same execution grid set up earlier */
	matrix_multiply_kernel_optimized<<< grid, threads >>>(Pd.elements, Md.elements, Nd.elements, MATRIX_SIZE);
	cudaDeviceSynchronize();
...
/* Check correctness of reference and device results */
