 ...
 * scheduler.  Keep in mind that if a signal is sent to a process
 * multiple times while the signal is masked, the signal will only be
 * received one time when the signal is later unmasked by the process.
 * This behavior is what is meant when people say that masked signals
 * are "pending" and not "queued".
 *
