...
int main (int argc, char** argv)
{
    ...
    /* Each producer thread will produce a work unit that will be
     * processed by the consumer thread.  In this example, each producer
     * will simply produce a result value and the consumer will keep a
     * running sum of these result values as they are produced.*/
    ...
    for (i=0; i<NUM_PRODUCERS; i++) {
        thread_num[i] = i;
        ret = pthread_create(&thread_id[i], NULL, producer, &thread_num[i]);
        if (ret) {
