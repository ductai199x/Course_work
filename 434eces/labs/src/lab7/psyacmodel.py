...
def f_SP_dB(maxfreq, nfilts):
    ...
    spreadingfunctionBarkdB[0:nfilts] = np.linspace(
        -maxbark*27, -8, nfilts)-23.5
    # "nfilts" bands for upper slope:
    spreadingfunctionBarkdB[nfilts:2 *
                            nfilts] = np.linspace(0, -maxbark*12.0, nfilts)-23.5
...
def maskingThresholdBark(mXbark, spreadingfuncmatrix, alpha, fs, nfilts):
    ...
    LTQ = np.clip((3.64*(f/1000.)**-0.8 - 6.5*np.exp(-0.6 *
                                                     (f/1000.-3.3)**2.)+1e-3*((f/1000.)**4.)), -20, 160)
...
def mappingfrombarkmat(W, nfft):
    ...
    # nfft: : number of subbands in fft
    nfreqs = int(nfft/2)
    W_inv = np.dot(np.diag((1.0/(np.sum(W, 1)+1e-6))**0.5),
                   W[:, 0:nfreqs + 1]).T
    return W_inv
...
if __name__ == '__main__':
    ...
    spreadingfuncmatrix = spreadingfunctionmat(
